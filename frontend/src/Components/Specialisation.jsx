...
const Specialisation = () => {
  ...
  useEffect(() => {
    if (data) {
      const specialisationBlogs = data.filter(
        (blog) => blog.category?.categoryName === "Specialisation"
      );
     
      setFilteredBlogs(specialisationBlogs.slice(0, 4)); 
    }
  }, [data]);
  ...
  return (
    <div className="mt-5 px-[150px]">
      <div className="w-full bg-[#b0764d] flex justify-center py-4 shadow-md">
        <h1 className="text-2xl font-bold text-white uppercase">Specialisation</h1>
      </div>
